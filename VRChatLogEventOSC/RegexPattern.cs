...
namespace VRChatLogEventOSC
{
    public static class RegexPattern
    {
        ...
        private static readonly Dictionary<EventTypeEnum, IEnumerable<string>> CaptureName = new(){
            ...
            {EventTypeEnum.MetPlayer, Enumerable.Empty<string>()},
            // {EventTypeEnum.JoinedRoom1, Enumerable.Empty<string>()},
            {EventTypeEnum.JoinedRoom1, new[]{"WorldURL", "WorldID", "InstanceID", "InstanceType", "UserID", "ReqInv", "Region"}},
            {EventTypeEnum.JoinedRoom2, new[]{"WorldName"}},
            ...
            {EventTypeEnum.AcceptInvite, Enumerable.Empty<string>()},
            ...
            {EventTypeEnum.JoinedRoom1Detail, new[]{"WorldURL", "WorldID", "InstanceID", "InstanceType", "UserID", "ReqInv", "Region"}},
            {EventTypeEnum.AcceptInviteDetail, new[]{"UserName", "UserID","WorldURL","WorldID","InstanceID","InstanceType","WorldUserID","ReqInv","Region","WorldName","Message",}},
            {EventTypeEnum.NotificationEvent, Enumerable.Empty<string>()},
            // {EventTypeEnum.TookScreenshot, new[]{"Path"}},
            {EventTypeEnum.TookScreenshot, Enumerable.Empty<string>()},
        };

        public static IEnumerable<string> CaptureNames(EventTypeEnum eventType)
        {
            return CaptureName[eventType];
        }
        ...
        public static IReadOnlyDictionary<EventTypeEnum, Regex> Regexes => new Dictionary<EventTypeEnum, Regex>()
        {
            ...
            // {EventTypeEnum.MetPlayer, MetPlayerRegex},
            // {EventTypeEnum.JoinedRoom1, JoinedRoom1Regex},
            {EventTypeEnum.JoinedRoom2, JoinedRoom2Regex},
            ...
            {EventTypeEnum.JoinedRoom1Detail, JoinedRoom1DetailRegex},
            {EventTypeEnum.JoinedRoom1, JoinedRoom1DetailRegex},
            {EventTypeEnum.AcceptInviteDetail, AcceptInviteDetailRegex},
            // {EventTypeEnum.NotificationEvent, NotificationEventRegex},
    ...
    static RegexPattern()
    {
        ...
        string joinedRoom1DetailPattern = @"\[(RoomManager|[Ǆǅ]*|Behaviour)\] Joining (?<WorldURL>(?<WorldID>wrld_[0-9a-zA-Z-]+):(?<InstanceID>[0-9]+)?~?(?<InstanceType>((private)|(friends)|hidden))?(\((?<UserID>(.{40}))\))?(?<ReqInv>~canRequestInvite)?~region\((?<Region>.+)\).+)$";
        ...
        // string notificationEventPattern = @"(?<Transceive>Received|Send|AcceptNotification for) [Nn]otification:.+type:(?<InstanceType>(request)?[Ii]nvite(Response)?|friendRequest),.+$";
        ...
        // string joinedRoom1DetailSimple = @"\[(RoomManager|[Ǆǅ]*|Behaviour)\] Joining (?<WorldURL>(?<WorldID>wrld_[0-9a-zA-Z-]+):(?<InstanceID>[0-9]+)?~?(?<InstanceType>((private)|(friends)|hidden))?(\((?<UserID>(.{40}))\))?(?<ReqInv>~canRequestInvite)?~region\((?<Region>.+)\).+)$";
        // string acceptInviteDetailSimple = @"AcceptNotification for notification:<Notification from username:(?<UserName>(.+)), sender user id:(?<UserID>(.{40})).+ of type: invite, id: (.{40}).+worldId=(?<WorldURL>(?<WorldID>wrld_[0-9a-zA-Z-]+):(?<InstanceID>[0-9]+)?~?(?<InstanceType>((private)|(friends)|hidden))?(\((?<WorldUserID>(.{40}))\))?(?<ReqInv>~canRequestInvite)?~region\((?<Region>.+)\).+), worldName=(?<WorldName>(.+?))(, inviteMessage=(?<Message>(.+?)))?(, imageUrl=(.+?))?}}, type:invite,.+$";
        // string notificationEventSimple = @"(?<Transceive>Received|Send|AcceptNotification for) [Nn]otification:.+type:(?<InstanceType>(request)?[Ii]nvite(Response)?|friendRequest),.+$";
